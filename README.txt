...
The program allows configuration of many aspects of the neural network. This includes the hashing range, n-gram length, and more. The first option you are offered is whether or not you want to configure all of these options yourself, or use the "optimized default parameters" which I have selected. The optimized defaults exist as a benchmark of the program's best performing neural network configuration, trainable in a short amount of time. This configuration is expected to train to ~90% in well under 3 minutes (time may vary on different hardware). Higher accuracies are possible if a different configuration is used. However, the training time would increase.
...
- Initially, I found it very difficult to train the network beyond "guessing" accuracy. This may have been because I wasn't using a good topology or training data, and/or because my code contained bugs which meant that the network wasn't training on meaningful data.
...
- Multiple hashed n-gram feature vectors can be used as input at once, and I have found this to improve the network's accuracy. For example, two hashed n-gram feature vectors of size 300 can be used back to back as input, with the first vector representing 1-grams and the second vector representing 2-grams. This comes to a total of 600 input neurons.
- Using hashed n-gram frequency vectors of size 300 seems to work well for my network, with quite a bit of leeway either way. Choosing this is a careful balance: if the vector is too small, you will have too many collisions, but if the vector is too big then the data becomes too "sparse", lowering the network's accuracy and increasing it's training time. Because of the above, this means that a total of ~600 input neurons seems about right for the network.
- Usage of n-grams of size >2 is detrimental. This must mean that there are too many permutations of n-gram for sizes above 3, relative to the length and number of language samples. The network has to find matching patterns of n-grams between languages to make good predictions, and it seems like n-grams of size 3 or greater are too "unique" to be useful.
...
- As for activation functions, I have found tanh() to be quite effective on my hidden layer, while ReLU also performs reasonably well. I believe the reason that tanh works well for me might be because of how my input data is normalized - the vector sums to 1, so increasing values at a vector index translates to an exponential increase in "hits" for that index, which matches the sharp curve of the tanh activation function. SoftMax should be used for the output layer, as the fact that all output neurons then sum to 1 make it very easy to extract each languages classification probability.
- I tried various metrics to decide the number of neurons in the hidden layer. I have found that, generally, "(input + output) / 4" is a good formula to use. A sizable number of neurons is needed here, to provide enough information for the large 235 neuron output of the network.
- I found a high dropout value to be very effective in improving the network's accuracy as well. I believe this also suggests that there is a lack of training data to effectively train the network - using a high dropout value prevents the network from overfitting, allowing it to make more use of the small training set.
...
With those findings in mind, here is the overall topology of the neural network that I chose:
...
The key conclusions and "learning outcomes" I make from these findings are:
...
- Training data is key for the training of neural networks, especially for deep learning. With limited data (~50 samples per input class), the network cannot realistically achieve a very high prediction accuracy on unseen data, and deep learning (use of several hidden layers) is very difficult to achieve.
...
- Computing the cosine distances of a sample's hashed n-gram distribution to the combined vectors (as described above) for all languages. In the previous language detection project, I found that cosine distance was a very good metric for detecting languages in this way. Perhaps the network could spot patterns in the cosine distance matrix, and be able to make predictions with an even higher precision than the cosine distance metric itself? To my surprise, I wasn't able to get any success with this method at all. I suspect there may have been a bug somewhere, but I didn't want to spend any more time in pursuing the method.

== Assumptions/Clarifications ==
- Without using negative training data, I could not calculate the sensitivity or specificity of the network. In place of this, the validation stage of the program calculates the accuracy and error of the network, and calculates the precision of the network in predicting each of the 235 languages. This is useful to see which of the languages the network has no problem predicting, and which languages it can't seem to tell apart from the others. For example, it struggles with Spanish, but has no problem predicting Portuguese. This information would be key to improving the network's accuracy even further.
...
- The option to use multiple n-gram vectors at once as input, configurable through the above extra.
